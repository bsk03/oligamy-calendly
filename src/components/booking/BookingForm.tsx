...
function formatTime(isoString: string, intlLocale: string, tz: string): string {
...
export function BookingForm({
	...
	if (createBooking.isSuccess) {
			...
			>
				...
				<div className="mt-2 rounded-lg border px-6 py-4 text-left text-sm">
					...
					<div className="mt-1.5 flex items-center gap-2">
						...
						<span>
							{formatTime(slot.start, intlLocale, timezone)} –{" "}
							{formatTime(slot.end, intlLocale, timezone)}
						</span>
					</div>
				</div>
		...
		if (noCard) return successContent;
