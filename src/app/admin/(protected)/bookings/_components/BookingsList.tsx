...
type BookingItem = RouterOutputs['booking']['list'][number];
...
export function BookingsList({ isAdmin }: { isAdmin: boolean }) {
	...
	const autoSync = api.booking.syncFromCalendar.useMutation({
		onSuccess: (data) => {
			if (data.synced > 0) {
				void utils.booking.list.invalidate();
				void utils.booking.stats.invalidate();
	...
	return (
		<div className='space-y-6'>
			...
			{/* Past */}
			{past.length > 0 && (
				<Section title='Past' bookings={past} isAdmin={isAdmin} emptyText='' />
			)}
